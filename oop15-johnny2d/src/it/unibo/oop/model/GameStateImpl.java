...
public final class GameStateImpl implements GameState {
    ...
    protected void initialize(final int levelNumber) {
    	...
    	//The screen dimension should be passed by the controller (POSSIBLE BUG FOR THE SCORE)
    	this.johnnyCharacter = Optional.ofNullable(Factory.MainCharacterFactory.generateStillCharacter(this.getArena().getPlayableRectangle().getCenterX(), this.getArena().getPlayableRectangle().getCenterY()));
    ...
    /**
     * Updates the {@link MainCharacter} basing the new movement on the keys pressed
     */
	...
	/**
	 * Add to the stableList the stable entities like {@link HealthBonus} and {@link Wall}
	 */
	public void addStableEntity(final AbstractEntity newEntity){
		this.stableList.add(newEntity);
	}
	/**
	 * Gets a {@link List} of {@link AbstractEntity} that contains the {@link Entity} that can't be moved
	 */
	public List<AbstractEntity> getStableList() {
		return new ArrayList<>(this.stableList);
	}
	/**
	 * Gets a {@link List} of {@link MovableEntity} that contains the {@link Entity} that can be moved
	 */
